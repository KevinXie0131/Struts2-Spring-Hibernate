...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			url : 'equipAction!datagrid.action',
	...
	function append() {
		var p = parent.sy.dialog({
			title : 'Add equipment',
			href : '${pageContext.request.contextPath}/equipAction!equipAdd.action',
			width : 500,
			height : 200,
			buttons : [ {
				text : 'Add',
				handler : function() {
					var f = p.find('form');
					f.form('submit', {
						url : '${pageContext.request.contextPath}/equipAction!add.action',
						success : function(d) {
	...
	function edit() {
		...
		if (rows.length == 1) {
			var p = parent.sy.dialog({
				title : 'Edit equip',
				href : '${pageContext.request.contextPath}/equipAction!equipEdit.action',
				width : 500,
				height : 200,
				buttons : [ {
					text : 'Edit',
					handler : function() {
						var f = p.find('form');
						f.form('submit', {
							url : '${pageContext.request.contextPath}/equipAction!edit.action',
							success : function(d) {
								var json = $.parseJSON(d);
								if (json.success) {
									datagrid.datagrid('reload');
		...
		if (rows.length > 0) {
			parent.sy.messagerConfirm('Confirm', 'Are you sure to delete', function(r) {
				if (r) {
					...
					$.ajax({
						url : '${pageContext.request.contextPath}/equipAction!delete.action',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
...
<body class="easyui-layout" data-options="fit:true">
	<div data-options="region:'north',border:false,title:'Search'" style="height: 60px;overflow: hidden;" align="left">
		<form id="searchForm">
			...
			<a href="${pageContext.request.contextPath}/equipAction!exportToExcel.action" class="easyui-linkbutton" data-options="iconCls:'icon-back',plain:true" onclick="exportToExcel();">Export to Excel</a>
		</form>
	</div>
